...
//   - Submit fees go straight into the Safe in the same transaction. The Safe is a plain Safe: two of three owners move
//     its ETH at any time; no game contract can hold, freeze or pull it.
//   - The Stream holds only what the Safe chose to pour. Exit: the Safe pauses it at once (it is the guardian), the
...
//   NET=robinhood node scripts/emergency.js pause      1 file: pause Mine, Workshop, Souls, Stream, Alchemists (instant)
...
const build = {
  ...
  pause(A) {
    return ["Mine", "Workshop", "Souls", "Stream", "Alchemists"].filter((n) => A[n]).map((n) => ({ to: A[n], value: "0", data: I.encodeFunctionData("pause"), what: `${n}.pause()` }));
  },
  // everything the game contracts hold goes back to the Safe; the stream must be paused first and closes for good
  rescue(A, safe, delay, salt) {
    const calls = [];
    if (A.Stream) calls.push({ to: A.Stream, data: I.encodeFunctionData("rescue", [safe]), what: `Stream.rescue(${safe})` });
  ...
  unpause(A, delay, salt, { streamClosed = false } = {}) {
    const names = ["Mine", "Workshop", "Souls"].concat(streamClosed ? [] : ["Stream"]).filter((n) => A[n]);
...
if (require.main === module) {
  ...
  (async () => {
    ...
    if (kind === "status") {
      ...
      for (const n of ["Mine", "Workshop", "Souls", "Stream", "Alchemists"]) {
        if (!A[n]) continue;
        const paused = await view(A[n], ["function paused() view returns (bool)"]).paused();
        console.log(`${n.padEnd(11)} ${A[n]}  ${paused ? "PAUSED" : "open  "}  ${await eth(A[n])} ETH${n === "Stream" && closed ? "  CLOSED (rescued)" : ""}`);
      }
      return;
    }
    ...
    if (kind === "pause") {
      ...
      const b = kind === "rescue" ? build.rescue(A, safe, delay, salt) : build.unpause(A, delay, salt, { streamClosed: closed });
      ...
      if (kind === "rescue") console.log(`\nThe stream must be paused before step 2 runs (the pause batch). Step 2 returns everything the stream, the mine and the summoning hold to the Safe ${safe}; the stream then stays closed.`);
    } else {
